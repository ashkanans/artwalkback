...
class TestAppChartIds(TestCase):
    def setUp(self):
    ...
    def test_get_app_chart_id(self):
        response = self.client.post(f'/api/app/chartId',
                                    json={},
    ...
    def test_get_sorted_app_chart_id(self):
        response = self.client.post('/api/app/chartId?sort_by=level4&sort_order=asc',
                                    json={},
                                    headers={'Authorization': "Bearer " + self.token})

        self.assertEqual(response.status_code, 200)

        response_data = response.json.get('ارزي', [])
